...
def main():
    ...
    sensor_tmp = BME280(t_mode=BME280_OSAMPLE_8, p_mode=BME280_OSAMPLE_8, h_mode=BME280_OSAMPLE_8, standby=BME280_STANDBY_250, filter=BME280_FILTER_off, address=0x77)
    degrees = sensor_tmp.read_temperature()
    pascals = sensor_tmp.read_pressure()
    humidity = sensor_tmp.read_humidity()
    print("log: temperature {}".format(degrees))
    print("log: pressure {}".format(pascals))
    print("log: humidity {}".format(humidity))
